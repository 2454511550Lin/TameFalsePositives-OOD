        ...
        for score in lst_scores: 
            ...
            for window in lst_windows:
                ...
                for hoeffding in [True, False]:
                    ...
                    if(not os.path.exists(f'../../plots-supp/change-{id_ds}/')):
                        os.mkdir(f'../../plots-supp/change-{id_ds}/')

                    if(hoeffding):
                        if(not os.path.exists(f'../../plots-supp/change-{id_ds}/with-hoeffding/')):
                            os.mkdir(f'../../plots-supp/change-{id_ds}/with-hoeffding/')
                    else:
                        if(not os.path.exists(f'../../plots-supp/change-{id_ds}/without-hoeffding/')):
                            os.mkdir(f'../../plots-supp/change-{id_ds}/without-hoeffding/')

                    if(hoeffding):
                        plt.savefig(f'../../plots-supp/change-{id_ds}/with-hoeffding/real_change_{key}_plot_window_{window}_gamma_{gamma}_hfding.png',dpi=150,bbox_inches='tight')
                    else:
                        plt.savefig(f'../../plots-supp/change-{id_ds}/without-hoeffding/real_change_{key}_plot_window_{window}_gamma_{gamma}.png',dpi=150,bbox_inches='tight')
